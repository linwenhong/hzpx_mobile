      ...
      <div v-if="!loading" class="page-content">
        ...
        <div v-if="topicDetails.type != 2">
          <yd-radio-group v-model="answer[0]">
            <div class="answer-option" v-for="option in topicDetails.topic_options">
...
export default {
  ...
  methods: {
    ...
    selectType (type) {
      ...
      this.$router.push('/question?public=' + this.public + '&tabIndex=' + type);
      this.tabIndex = type;
      this.getTopics();
    },
    getTopics() {
      ...
      if (this.tabIndex == 2) { // 错题本
        this.$Service.getWrongTopics(request).then(response => {
          ...
          if (response.code == 200) {
            const hasTopic = (response.data.topics && response.data.topics[0]) ? true : false;
  ...
  destroyed () {
    this.$Cache.clear(['topics', 'topicIndex', 'topicCompleted', 'doCount', 'topicsCount', 'errorCount']);
  },
  created () {
